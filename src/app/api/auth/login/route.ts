...
import { authenticateLocalPreview } from "@/server/local-preview";

export async function POST(request: Request) {
  try {
    const credentials = loginSchema.parse(await request.json());
    const { token, expiresAt, user } = authenticateLocalPreview(credentials.email, credentials.password) ?? await authenticate(credentials.email, credentials.password);
